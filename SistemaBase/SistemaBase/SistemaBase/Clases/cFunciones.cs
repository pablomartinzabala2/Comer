...
namespace SistemaBase.Clases
{
    public  class cFunciones
    {
        ...
        {
            string Cadena = cConexion.GetConexion();
            string sql = "";
        ...
        public void CargarControlesxTabla(GroupBox Grupo, string Tabla, string CampoClavePrimaria, string ValorClavePrimaria)
        {
            string nombre;
            string sql = "select * from " + Tabla;
            sql = sql + " where " + CampoClavePrimaria + "=" + ValorClavePrimaria.ToString();
            ...
            {
                nombre = g.Name;
                if (nombre.Substring(0, 3).ToUpper () == "TXT")
                {
                    TextBox CajaTexto = (TextBox)g;
                    Col = CajaTexto.Tag.ToString();
                    foreach (DataColumn  Columna in trdo.Columns)
                    {
                        if (Columna.ToString ()==Col)
                        {
                            CajaTexto.Text = trdo.Rows[0][Col].ToString();
                        }
                    }
                }
                
                if (nombre.Substring(0, 3).ToUpper() == "CMB")
                {
                    ComboBox Combo = (ComboBox)g;
                    Col = Combo.Tag.ToString();
                    foreach (DataColumn Columna in trdo.Columns)
                    {
                        if (Columna.ToString() == Col)
                        {
                            Combo.SelectedValue = trdo.Rows[0][Col].ToString();
                        }
                    }
                }
